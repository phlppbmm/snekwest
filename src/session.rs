...
impl Session {
        ...
        if let Some(timeout_params) = &params.timeout {
            match timeout_params {
                ...
                TimeoutParameter::Pair(connect, Some(read)) => {
                    ...
                    request =
                        request.timeout(std::time::Duration::from_secs_f64(connect_secs + *read));
                }
    ...
    fn extract_response_headers(response: &reqwest::blocking::Response) -> Vec<(String, String)> {
        response
            .headers()
            .iter()
    ...
    fn extract_response_cookies(response: &reqwest::blocking::Response) -> HashMap<String, String> {
    ...
    fn do_request(&self, mut params: RequestParams) -> PyResult<RawResponseData> {
        ...
        let had_explicit_connect_timeout =
            matches!(&params.timeout, Some(TimeoutParameter::Pair(Some(_), _)));

        let has_proxies = params.proxies.as_ref().is_some_and(|p| !p.is_empty());
        ...
        loop {
            ...
            if is_redir && params.allow_redirects {
                ...
                if (status == 301 && current_method.to_uppercase() == "POST")
                    || (matches!(status, 302 | 303) && current_method.to_uppercase() != "HEAD")
                {
...
impl Session {
    ...
    fn close(&self, py: Python<'_>) -> PyResult<()> {
        ...
        if let Ok(values) = adapters.call_method0("values") {
            let values_as_list = builtins
                .and_then(|b| b.getattr("list").ok())
    ...
    fn mount(&self, py: Python<'_>, prefix: String, adapter: Py<PyAny>) -> PyResult<()> {
        ...
        let keys_list: Vec<String> = builtins
            .getattr("list")?
    ...
    fn get_adapter(&self, py: Python<'_>, url: String) -> PyResult<Py<PyAny>> {
        ...
        let builtins = py.import("builtins")?;
        let items_list: Vec<(String, Py<PyAny>)> = builtins
            .getattr("list")?
        ...
        Err(PyErr::from_value(invalid_schema.call1((format!(
    ...
    fn prepare_request(&self, py: Python<'_>, request: &Bound<'_, PyAny>) -> PyResult<Py<PyAny>> {
        ...
        let req_cookies = if req_cookies.is_none()
            || (req_cookies.is_instance_of::<PyDict>() && req_cookies.len()? == 0)
        {
        ...
        let merged_headers = merge_setting.call(
            (request.getattr("headers")?, &self.headers.bind(py)),
            Some(&kwargs_dict),
        )?;
        let merged_params =
            merge_setting.call1((request.getattr("params")?, &self.params.bind(py)))?;
        let merged_auth = merge_setting.call1((&auth, &self.auth.bind(py)))?;
        let merged_hooks =
            merge_hooks_fn.call1((request.getattr("hooks")?, &self.hooks.bind(py)))?;
    ...
    fn merge_environment_settings(
        ...
        if self.trust_env {
            ...
            let env_proxies = utils
                .getattr("get_environ_proxies")?
                .call((&url,), Some(&no_proxy_kw))?;
            // setdefault for each env proxy
            let env_items = env_proxies.call_method0("items")?;
            let builtins = py.import("builtins")?;
            let env_items_list: Vec<(String, Py<PyAny>)> =
                builtins.getattr("list")?.call1((&env_items,))?.extract()?;
    ...
    fn request(
        ...
        req_build_kwargs.set_item(
            "headers",
            headers
                .as_ref()
                .map_or_else(|| py.None(), |h| h.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "files",
            files
                .as_ref()
                .map_or_else(|| py.None(), |f| f.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "data",
            data.as_ref()
                .map_or_else(|| PyDict::new(py).into_any().unbind(), |d| d.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "json",
            json.as_ref().map_or_else(|| py.None(), |j| j.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "params",
            params
                .as_ref()
                .map_or_else(|| PyDict::new(py).into_any().unbind(), |p| p.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "auth",
            auth.as_ref().map_or_else(|| py.None(), |a| a.clone_ref(py)),
        )?;
        req_build_kwargs.set_item(
            "cookies",
            cookies
    ...
    fn get(
        ...
        let kw = kwargs
            .map(|d| d.copy())
            .transpose()?
            .unwrap_or_else(|| PyDict::new(py).clone());
        if !kw.contains("allow_redirects")? {
            kw.set_item("allow_redirects", true)?;
        }
        slf.call_method("request", ("GET", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn options(
        ...
        slf.call_method("request", ("OPTIONS", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn head(
        ...
        slf.call_method("request", ("HEAD", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn post(
        ...
        kw.set_item(
            "data",
            data.as_ref().map_or_else(|| py.None(), |d| d.clone_ref(py)),
        )?;
        kw.set_item(
            "json",
            json.as_ref().map_or_else(|| py.None(), |j| j.clone_ref(py)),
        )?;
        slf.call_method("request", ("POST", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn put(
        ...
        slf.call_method("request", ("PUT", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn patch(
        ...
        slf.call_method("request", ("PATCH", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn delete(
        ...
        let kw = kwargs
            ...
            .transpose()?
            .unwrap_or_else(|| PyDict::new(py).clone());
        slf.call_method("request", ("DELETE", url), Some(&kw))
            .map(|r| r.unbind())
    }
    ...
    fn send(
        ...
        let allow_redirects: bool = kwargs
